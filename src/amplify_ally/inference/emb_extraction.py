    ...
    **kwargs,
    ...
    with torch.no_grad():
        for global_id, x, _, pad_mask in tqdm(dataloader, desc="Extract embeddings", unit="batch"):
            ...
            # init SWE_Pooling lazily on first batch so we have emb shape
            if pooling_method == "swe" and swe_pooling is None:
                _, max_length, hidden_size = emb.shape
    ...
    return torch.cat(embedding, dim=0)  # [n_samples, emb_dim] on CPU