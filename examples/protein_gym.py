        ...
        for prot in tqdm.tqdm(prots):
            ...
            for chunk in sequence_chunks:
                for p in [chunk, chunk[::-1]]:
                    ids = torch.as_tensor(tokenizer.encode(p)).to(torch.long).unsqueeze(0).to(device)
                    input_ids = ids[:, :-1]
                    targets   = ids[:, 1:]
...
def main():
    ...
    parser.add_argument('--model_path', default="/n/groups/marks/projects/marks_lab_and_oatml/protein_transformer/baseline_models/RITA/small", type=str, help='Path to pretrained model')
    parser.add_argument('--config_path', default="/n/groups/marks/projects/marks_lab_and_oatml/protein_transformer/baseline_models/RITA/small", type=str, help='Path to config file')
    parser.add_argument('--DMS_reference_file_path', default='/hpc/group/naderilab/eleanor/AMPLIFY_ALLY/ProteinGym/DMS_substitutions.csv', type=str, help='Path of DMS folder')
    parser.add_argument('--DMS_data_folder', default='/hpc/group/naderilab/eleanor/AMPLIFY_ALLY/ProteinGym/DMS_ProteinGym_substitutions', type=str, help='Path of DMS folder')
    parser.add_argument('--DMS_index', type=int, default=0, help='DMS index for computing')
    parser.add_argument('--output_scores_folder', default='/hpc/group/naderilab/eleanor/AMPLIFY_ALLY/ProteinGym/output', type=str, help='Name of folder to write model scores to')
    parser.add_argument('--indel_mode', action='store_true', help='Whether to score sequences with insertions and deletions') # False by default
    ...
    DMS_data[['mutant','ALLY_score','DMS_score']].to_csv(scoring_filename, index=False)
